                        ...
                        <div class="row restaurant_payout_create">
                            <div class="restaurant_payout_create-inner">
                                ...
                                <fieldset id="story_upload_div" style="display: none;">
                                    <legend>Story</legend>
                                    <div class="form-group row">
                                        <label class="col-12 control-label" style="color:red;font-size:15px;">{{trans('lang.image_video_note')}} </label>
                                    </div>
    ...
    <script>
                            ...
                            if (restaurant.specialDiscount[i]['timeslot'].length != 0) {
                                for (j = 0; j < restaurant.specialDiscount[i]['timeslot'].length; j++) {
                                    ...
                                    $('#special_offer_table_' + day + ' tr:last').after('<tr>' +
                                        ...
                                        '<td style="width:30%;"><select id="type' + day + j + i + '" class="form-control ' + i + '_' + j + '_row"><option value="delivery"/>{{trans('lang.delivery_discount')}}</option><option value="dinein"/>{{trans('lang.dineIn_discount')}}</option></select>' +
                                        '</td>' +
                                        '<td class="action-btn" style="width:20%;">' +
                                        '<button type="button" class="btn btn-primary ' + i + '_' + j + '_row"  onclick="updateMoreFunctionButton(`' + day + '`,`' + j + '`,`' + i + '`)" ><i class="mdi mdi-lead-pencil" title="{{trans('lang.edit')}}"></i></button>' +
                            ...
                            if (restaurant.workingHours[i]['timeslot'].length != 0) {
                                for (j = 0; j < restaurant.workingHours[i]['timeslot'].length; j++) {
                                    ...
                                    $('#working_hour_table_' + day + ' tr:last').after('<tr>' +
                                        ...
                                        '<td class="action-btn" style="width:20%;">' +
                                        '<button type="button" class="btn btn-primary ' + i + '_' + j + '_row workingHours_' + i + '_' + j + '"  onclick="updatehoursFunctionButton(`' + day + '`,`' + j + '`,`' + i + '`)" ><i class="mdi mdi-lead-pencil" title="{{trans('lang.edit')}}"></i></button>' +
        ...
        function handleStoryFileSelect(evt) {
            ...
            video.onloadedmetadata = function() {
                ...
                if (video.duration > storevideoDuration) {
                    $(".error_top").show();
                    $(".error_top").html("");
                    $(".error_top").append("<p>{{trans('lang.story_video_duration_maximum_allow_to')}} " + storevideoDuration + " {{trans('lang.seconds')}}</p>");
                    window.scrollTo(0, 0);
                    evt.target.value = '';
                    return false;
                }
        ...
        function addMoreButton(day, day2, count) {
            ...
            $('#special_offer_table_' + day + ' tr:last').after('<tr>' +
                ...
                '</td>' +
                '<td style="width:30%;"><select id="type' + day + count + '" class="form-control"><option value="delivery"/>{{trans('lang.delivery_discount')}}</option><option value="dinein"/>{{trans('lang.dineIn_discount')}}</option></select></td>' +
                '<td class="action-btn" style="width:20%;">' +
                '<button type="button" class="btn btn-primary save_option_day_button' + day + count + '" onclick="addMoreFunctionButton(`' + day2 + '`,`' + day + '`,' + countAddButton + ')" style="width:62%;"><i class="fa fa-save"></i> {{trans('lang.save')}}</button>' +
                '</td></tr>');
            countAddButton++;
        }
        ...
        function addMoreFunctionhour(day1, day2, count) {
            ...
            if (from == '') {
                $(".error_top").show();
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.please_enter_restaurant_open_time')}}</p>");
                window.scrollTo(0, 0);
            } else if (to == '') {
                $(".error_top").show();
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.please_enter_restaurant_close_time')}}</p>");
                window.scrollTo(0, 0);
            } else if (from > to) {
                $(".error_top").show();
                $(".error_top").html("");
