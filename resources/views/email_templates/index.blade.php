                    ...
                    refData.get().then(async function (querySnapshot) {
                        ...
                        paginatedRecords.forEach(function (childData) {
                            ...
                            records.push([
                                type,
                                childData.subject,
                                '<span class="action-btn"><a href="' + route1 + '"><i class="mdi mdi-lead-pencil" title="{{trans('lang.edit')}}"></i></a></span>'
                            ]);
                        });
                        $('#data-table_processing').hide(); // Hide loader
                        callback({
        ...
        function buildHTML(snapshots) {
            ...
            snapshots.docs.forEach(async (listval) => {
                ...
                html = html + '<td class="action-btn">' +
                    '<a href="' + route1 + '"><i class="mdi mdi-lead-pencil" title="{{trans('lang.edit')}}"></i></a></td>';
                html = html + '</tr>';
                count = count + 1;
            });
            return html;
