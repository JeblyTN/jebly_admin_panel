        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ trans('lang.canceled') }}</button>
                </div>
            </div>
        </div>
    </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        var refData = database.collection('restaurant_orders').where('isPosOrder','==',false);
        var ref = '';
        ...
        async function buildHTML(val) {
            ...
            html.push('<a href="' + route1 + '" class="redirecttopage" title="'+val.id+'">' + (val.id.substring(0,7)) + '</a>');
            if (userID) {
                var title = '';
            ...
            var actionHtml = '';
            actionHtml += '<span class="action-btn"><a href="' + printRoute + '"><i class="fa fa-print" style="font-size:20px;"></i></a><a href="' + route1 + '"><i class="mdi mdi-lead-pencil" title=" {{trans("lang.edit")}}"></i></a>';
            if (val.status === "Order Accepted" || val.status === "Driver Rejected") {
                actionHtml += '<a id="' + val.id + '" class="order-assign-btn" name="order-assign" href="javascript:void(0)"><i class="mdi mdi-truck-delivery"></i></a>';
            }
            if (checkDeletePermission) {
        ...
        async function assignDriverToOrder(orderId, driverId, driverNearByData, orderData) {
            ...
            let orderRef = database.collection('restaurant_orders').doc(orderId);
