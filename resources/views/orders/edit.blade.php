                    ...
                    database.collection('restaurant_orders').doc(id).update(updatedData).then(async function(result) {
                        ...
                        database.collection('wallet').doc(wId).set({
                            ...
                            if (orderTaxAmount != 0 || orderTaxAmount != '') {
                                var wId = database.collection('temp').doc().id;
                                database.collection('wallet').doc(wId).set({
                                    'amount': parseFloat(orderTaxAmount),
                                    'date': date,
                                    'id': wId,
                                    'isTopUp': true,
                                    'order_id': "<?php echo $id; ?>",
                            ...
                            database.collection('users').where('id', '==', vendorAuthor)
                                .get().then(async function(snapshotsnew) {
                                    ...
                                    if (vendordata) {
                                        ...
                                        newVendorWallet = vendorWallet + vendorAmount + parseFloat(orderTaxAmount);
        ...
        function buildHTMLProductsList(snapshotsProducts) {
            ...
            snapshotsProducts.forEach((product) => {
                ...
                if (globalTaxScope === "product" && tax.length > 0) {
                    ...
                    .map(t => {
                        const value = t.type === 'percentage' ? `${t.tax}%`: currencyAtRight ? `${t.tax} ${currentCurrency}`: `${currentCurrency} ${t.tax}`;
                        return `${t.title}(${value})`;
                    }).join(', ');
                    tax_titles = `<p class="d-block text-muted">Taxes: ${taxDisplay}</p>`;
                }
                html = html + '' + tax_titles;
