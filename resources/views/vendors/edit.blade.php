...
@section('scripts')
    ...
    <script>
                            ...
                            async function() {
                                await database.collection('users').doc(user_id)
                                    .update({
                                        ...
                                        jQuery("#data-table_processing").hide();
                                        Swal.fire('{{trans("lang.update_complete")}}',`{{trans("lang.user_updated")}}`,'success');
                                    });
                            });
                    }).catch(err => {
                        jQuery("#data-table_processing").hide();
        ...
        function formatState(state) {
            ...
            var phoneCode = $option.data('phonecode');
        
            if (!phoneCode || !newcountriesjs[phoneCode]) {
                return state.text;
            }
            var baseUrl = "<?php echo URL::to('/'); ?>/scss/icons/flag-icon-css/flags";
            var isoLower = $option.data('iso').toLowerCase();
        
            var $state = $(
                '<span><img src="' + baseUrl + '/' + isoLower + '.svg" class="img-flag" /> ' + state.text + '</span>'
            );
            return $state;
        }

        // Use the same for formatState2 (selection)
        function formatState2(state) {
            return formatState(state);  // Or duplicate the code
        }
