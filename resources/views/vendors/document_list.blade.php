                ...
                database.collection('documents_verify').doc(id).update({
                    ...
                    await ref.get().then( async function(snapshotsVendor){
                        if (snapshotsVendor.docs.length > 0) {
                            ...
                            if(verification){
                                if (status == "rejected") {
                                    var sendNotificationStatus = await sendNotification(fcmToken, '{{trans("lang.rejected_your_document")}}', '{{trans("lang.admin_is_rejected_your")}} ' + docTitle + ' . {{trans("lang.please_submit_again")}}');
