...
$(document).ready(function () {
    ...
    var vendor_restaurant_select = $("#vendor_restaurant_select").val();
    ...
    database.collection('coupons').where('code', '==', code).get().then(function (snapshot) {
        if (snapshot.size > 0) {
            $(".error_top").show();
            $(".error_top").html("");
            $(".error_top").append("<p>{{trans('lang.code_is_already_exist_try_another_one')}}</p>");
            window.scrollTo(0, 0);
        }
        else {
            if (code == '') {
                ...
                window.scrollTo(0, 0);
            }else if(vendor_restaurant_select == '' && resturant_id == ''){
                $(".error_top").show();
                $(".error_top").html("");
